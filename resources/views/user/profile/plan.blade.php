    ...
    <form {{-- action="{{ route('user.profile.upgrade-plan') }}" --}} method="post" class="col-12 col-md-9 d-flex flex-column showToastAbove">
        ...
        <div class="card-body position-relative">
            ...
            @include('include.admin.premium-overlay', [
                'message' => __('Subscription plan details and upgrade options are available in the paid version.'),
            ])
            ...
            <div class="blur-section">
                ...
                @if ($user->plan_recurring_at)
                    <button type="button" class="btn btn-outline-danger mb-3 disabled" data-bs-toggle="modal"
                        data-bs-target="#cancel-modal" disabled>{{ __('Cancel') }}</button>
                @endif

                @if (getAuthUserInfo('role') != 'admin' && showUpgrade())
                    <a href="{{ route('pricing') }}" class="disabled">
                        <button type="button" name="submit" class="btn btn-primary mb-3" disabled>{{ __('Upgrade') }}</button>
                    </a>
                @endif
            </div>
        </div>
