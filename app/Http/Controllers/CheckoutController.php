...
use App\Models\PaymentGatewayRelation;
...
class CheckoutController extends Controller
{
    ...
    public function index(Request $request, $id)
    {
        ...
        return view('checkout.index', ['page' => __('Checkout'), 'plan' => $plan, 'user' => $request->user(), 'taxRates' => $taxRates, 'coupon' => $coupon, 'inclTaxRatesPercentage' => $inclTaxRatesPercentage, 'exclTaxRatesPercentage' => $exclTaxRatesPercentage , 'countries' => $countries]);
    }
    ...
    private function initPaystack(Request $request, Plan $plan, $coupon, $taxRates, $amount)
    {
        ...
        try {
            ...
            $planInGatewayRelation = PaymentGatewayRelation::where('plan_id', '=', $plan->id)->where('amount', '=', $planAmount)->firstOrFail();
            ...
            try {
                ...
                if ($createPlan['status'] == true) {
                    ...
                    $planRelation = new PaymentGatewayRelation;
    ...
    private function initRazorpay(Request $request, Plan $plan, $coupon, $taxRates, $amount)
    {
        ...
        try {
            ...
            $planInGatewayRelation = PaymentGatewayRelation::where('plan_id', '=', $plan->id)->where('payment_gateway', 'RAZORPAY')->where('amount', '=', $planAmount)->firstOrFail();
            // Attempt to get the plan details
            $getPlan = callCurlApiRequest('/plans/' . $planInGatewayRelation->plan_code, 'GET', null, 'v1');
            ...
            try {
                ...
                $createPlan = callCurlApiRequest('/plans', 'POST', $fields, 'v1');
        ...
        try {
            ...
            $createSubscriptionWithTransaction = callCurlApiRequest('/subscriptions', 'POST', $fields, 'v1');
    ...
    private function initStripe(Request $request, Plan $plan, $coupon, $taxRates, $amount)
    {
        ...
        $stripePlan = $plan->id . '' . $request->input('interval') . '' . $stripeAmount . '_' . $plan->currency;
    ...
    private function initPayPal(Request $request, Plan $plan, $coupon, $taxRates, $amount)
    {
        ...
        $payPalPlan = 'plan_' . $plan->id . '' . $request->input('interval') . '' . $payPalAmount . '_' . $plan->currency;
